...
def render_kpis(df_todos: pd.DataFrame):
    ...
    with st.expander("⏰ Horário de Pico", expanded=False):
        ...
        _fig_h, _top3_cap, _top3_vnd, _cap_c, _vnd_c = grafico_horarios_pico(df_todos)
        ...
        with _h1:
            ...
            st.markdown(
                ...
                + "".join(
                    ...
                    f"{h:02d}h "
                    f"<span style='color:#e8eef8;font-weight:400;font-size:12px;'>({_cap_c[h]} leads)</span>"
                    f"</span>"
                    for h in _pico_cap
                )
                + "</div>",
        ...
        with _h2:
            ...
            st.markdown(
                ...
                + "".join(
                    ...
                    f"{h:02d}h "
                    f"<span style='color:#e8eef8;font-weight:400;font-size:12px;'>({_vnd_c[h]} vendas)</span>"
                    f"</span>"
                    for h in _pico_vnd
                )
                + "</div>",
